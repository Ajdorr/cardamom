...
func getOAuthRedirectURL(provider string) string {
	if !cfg.IsLocal() {
		return fmt.Sprintf("https://%s/auth/oauth-return/%s", cfg.C.Server.Domain, provider)
	} else {
		return fmt.Sprintf("http://%s:8080/auth/oauth-return/%s", cfg.C.Server.Domain, provider)
	}
}
