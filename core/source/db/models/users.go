...
func getSalt() []byte {
	return []byte(cfg.C.Server.PasswordSalt)
}
...
func GetUserByEmail(email string) (*User, error) {
	user := User{}
	if db := db.DB().Where(&User{Email: email}).Limit(1).First(&user); db.Error != nil {
		return nil, db.Error
	} else {
		return &user, nil
	}
...
func (u User) GetAccessToken() (string, string, error) {
	...
	token, err := jwt_ext.CreateJWT(AuthToken{
		BaseClaims: jwt.RegisteredClaims{
			Issuer:    cfg.C.Server.Domain,
			Subject:   "access_token",
			Audience:  []string{cfg.C.Server.Domain},
